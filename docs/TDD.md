...
- **Terrain texturé** (`TerrainMaterial.ts`) : matériau `MeshStandardMaterial` étendu par `onBeforeCompile` qui **splatte 4 matières** (herbe/sable/roche/terre) avec leurs **normal maps**, en UV planaires monde. Les poids par sommet (attribut `splat`, vec4) sont calculés dans `TerrainMesh.writeSplat` selon **biome + pente + altitude** (herbe sur prairies, sable sur côtes, terre sur pentes moyennes, roche sur fortes pentes/hauteurs). La couleur par sommet (dégradé chaud + terrasses) teinte le résultat. Textures dans `public/textures/terrain/` (placeholders générés par `tools/gen-placeholder-textures.mjs` ; remplacer les PNG suffit).
